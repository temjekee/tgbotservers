...
from utils import send_pdf_and_buttons, remove_temp_dir
import logging
...
# Настройка клиента HTTPX с увеличенными тайм-аутами
request = HTTPXRequest(read_timeout=60.0, connect_timeout=60.0)
bot = Bot(token=TOKEN, request=request)

@celery_app.task(name="generate_pdf_task")
def generate_pdf_task(url, temp_dir, chat_id):
    """Celery задача, вызывающая асинхронный процесс генерации PDF и отправки его клиенту."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        pdf_path = loop.run_until_complete(generate_pdf(url, Path(temp_dir)))
        if pdf_path:
            loop.run_until_complete(send_pdf_and_buttons(bot, chat_id, pdf_path, POST_PDF_BUTTONS))
            loop.run_until_complete(remove_temp_dir(Path(temp_dir)))  # Удаление временной директории после отправки
    except Exception as e:
        logging.error(f"Ошибка в generate_pdf_task: {e}")
    finally:
        loop.run_until_complete(loop.shutdown_asyncgens())
        loop.close()
    return str(pdf_path)

async def generate_pdf(url, temp_dir: Path):
    try:
        async with async_playwright() as p:
            ...
            # Переход по переданному URL
            await page.goto(url, wait_until='networkidle', timeout=60000)  # Увеличение тайм-аута до 60 секунд
